...
export LAUNCHPAD_CREDENTIALS_ACCESS_TOKEN='pPs367Km5glpfXXK9BQl'
export LAUNCHPAD_CREDENTIALS_ACCESS_SECRET='PQ0r1JGdHQRWQCS255jNZdglCcK4KDmsGRSZxl4CZwWvMF00GLRFbz1185L08cjh5zWqhQddsvmgLg7l'
...
if [ -z "$GITLAB_CI" ]; then
  ...
  echo "FROM ghcr.io/canonical/snapcraft:8_core24" > Dockerfile
  ...
  /kaniko/executor --context $CI_PROJECT_DIR --dockerfile $CI_PROJECT_DIR/Dockerfile --destination $CI_REGISTRY_IMAGE:build-snap-noble-${RUNNER} --cache=true --cache-ttl=120h --image-fs-extract-retry 1 --verbosity=warn
fi 
