...
OLD_SNAP=$SNAP
SNAP=$(echo $SNAP | sed 's/^temp-//')


# Finish .snap to be exposed as artifact
printf "\e[0Ksection_start:`date +%s`:snap_making[collapsed=true]\r\e[0KPackaging prime as ${SNAP}\n"
mv ${OLD_SNAP} ${SNAP}
printf "(INFO): Suceeded. To test this build, install it from the artifact with: sudo snap install --dangerous ${SNAP}\n"
printf "\e[0Ksection_end:`date +%s`:snap_making\r\e[0K\n"


# Generate shasums for .snap
printf "\e[0Ksection_start:`date +%s`:snap_trust[collapsed=true]\r\e[0KChecksumming ${SNAP}\n"
printf "(INFO): ${SNAP} SHA-256: $(sha256sum ${SNAP} | cut -d ' ' -f 1)\n"
printf "(INFO): ${SNAP} SHA-512: $(sha512sum ${SNAP} | cut -d ' ' -f 1)\n"
printf "\e[0Ksection_end:`date +%s`:snap_trust\r\e[0K\n"
...
if [ "$GITLAB_CI" ] && [ "$CI_COMMIT_BRANCH" = "$CI_DEFAULT_BRANCH" ]; then
  printf "\e[0Ksection_start:`date +%s`:flat_publish[collapsed=true]\r\e[0KPublishing snap to Snap Store\n"
