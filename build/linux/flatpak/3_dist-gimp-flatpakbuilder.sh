...
if [ "$GITLAB_CI" ]; then
  ...
  tar xf repo-${RUNNER:-$(uname -m)}.tar -C repo/ --warning=no-timestamp
fi


# CONSTRUCT .FLATPAK
# Flatpak "bundle" to be tested with GNOME runtime installed
# (it is NOT a real/full bundle, deps from GNOME runtime are not bundled)
printf "\e[0Ksection_start:`date +%s`:flat_making[collapsed=true]\r\e[0KPackaging repo as ${APP_ID}-${ARCH}.flatpak\n"
mv temp_${APP_ID}-${ARCH}.flatpak ${APP_ID}-${ARCH}.flatpak

printf "\e[0Ksection_end:`date +%s`:flat_making\r\e[0K\n"


# GENERATE SHASUMS FOR .FLATPAK
printf "\e[0Ksection_start:`date +%s`:flat_trust[collapsed=true]\r\e[0KChecksumming ${APP_ID}-${ARCH}.flatpak\n"
printf "(INFO): ${APP_ID}-${ARCH}.flatpak SHA-256: $(sha256sum ${APP_ID}-${ARCH}.flatpak | cut -d ' ' -f 1)\n"
printf "(INFO): ${APP_ID}-${ARCH}.flatpak SHA-512: $(sha512sum ${APP_ID}-${ARCH}.flatpak | cut -d ' ' -f 1)\n"
